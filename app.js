...
document.addEventListener('DOMContentLoaded', () => {
    // Check if the page is being loaded inside an iframe on a state page
    const params = new URLSearchParams(window.location.search);
    const stateCodeFromUrl = params.get('state');
    
    if (stateCodeFromUrl) {
        const stateSelect = document.getElementById('state-select');
        const stateSelectionGroup = document.getElementById('state-selection-group');
        
        // Use a small delay to ensure the original script has populated the dropdown
        setTimeout(() => {
            if (stateSelect) {
                stateSelect.value = stateCodeFromUrl;
                
                // Hide the state dropdown to prevent users from changing it on a state-specific page
                if (stateSelectionGroup) {
                    stateSelectionGroup.style.display = 'none';
                }
            }
